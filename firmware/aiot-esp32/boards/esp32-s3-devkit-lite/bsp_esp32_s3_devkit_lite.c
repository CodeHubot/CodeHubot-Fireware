/**
 * @file bsp_esp32_s3_devkit_lite_lite.c
 * @brief ESP32-S3 DevKit Lite 板级支持包实现（含雨水传感器）
 * 
 * @author AIOT Team
 * @date 2024
 */

#include "bsp_esp32_s3_devkit_lite.h"
#include "board_config.h"
#include "../../main/bsp/bsp_interface.h"
// #include "../../main/bluetooth/bt_provision.h"  // 临时禁用
#include "../../drivers/sensors/dht11.h"      // DHT11传感器驱动
// Lite板子不使用DS18B20，GPIO39用于雨水传感器
...
// 静态函数声明
static hal_err_t esp32_s3_devkit_lite_init_leds(void);
static hal_err_t esp32_s3_devkit_lite_init_buttons(void);
static hal_err_t esp32_s3_devkit_lite_init_relays(void);
static hal_err_t esp32_s3_devkit_lite_init_servos(void);
static hal_err_t esp32_s3_devkit_lite_led_control(uint8_t led_index, bool state);
static hal_err_t esp32_s3_devkit_lite_led_set_brightness(uint8_t led_index, uint8_t brightness);
static hal_err_t esp32_s3_devkit_lite_relay_control(uint8_t relay_index, bool state);
static hal_err_t esp32_s3_devkit_lite_servo_set_angle(uint8_t servo_index, uint16_t angle);

// 蓝牙配网功能函数声明 - 临时禁用
/*
static hal_err_t esp32_s3_devkit_lite_bt_provision_init(const bt_provision_config_t* config);
static hal_err_t esp32_s3_devkit_lite_bt_provision_deinit(void);
static hal_err_t esp32_s3_devkit_lite_bt_provision_start(void);
static hal_err_t esp32_s3_devkit_lite_bt_provision_stop(void);
*/
...
// 传感器显示信息列表 - 用于LCD动态UI（Lite板子只有DHT11）
...
static hal_err_t esp32_s3_devkit_lite_init(void)
{
    printf("BSP: Initializing ESP32-S3 DevKit...\n");
    
    // 1. 初始化LED GPIO
    hal_err_t ret = esp32_s3_devkit_lite_init_leds();
    if (ret != HAL_OK) {
    ...
    // 2. 初始化按键GPIO
    ret = esp32_s3_devkit_lite_init_buttons();
    if (ret != HAL_OK) {
    ...
    // 3. 初始化继电器GPIO
    ret = esp32_s3_devkit_lite_init_relays();
    if (ret != HAL_OK) {
        printf("BSP: Failed to initialize relays\n");
        return ret;
    }
    
    // 4. Lite板子没有舵机，跳过舵机初始化
    // ret = esp32_s3_devkit_lite_init_servos();
    
    printf("BSP: ESP32-S3 DevKit initialized successfully\n");
    return HAL_OK;
}

static hal_err_t esp32_s3_devkit_lite_deinit(void)
{
...
static const bsp_board_info_t* esp32_s3_devkit_lite_get_board_info(void)
{
    return &s_board_info;
}

static const bsp_hardware_config_t* esp32_s3_devkit_lite_get_hardware_config(void)
{
    // 动态初始化硬件配置
...
// 传感器控制函数
static hal_err_t esp32_s3_devkit_lite_sensor_init(void)
{
    printf("BSP: Initializing sensors (DHT11 for Lite board)...\n");
    ...
    // Lite板子只有DHT11传感器
    printf("BSP: ℹ️ Lite板子：仅支持DHT11传感器\n");
    ...
    if (dht11_ok) {
        ...
        printf("BSP: ⚠️ DHT11 failed to initialize - 系统将继续运行\n");
...
static hal_err_t esp32_s3_devkit_lite_sensor_deinit(void)
{
...
static hal_err_t esp32_s3_devkit_lite_sensor_read(uint8_t sensor_id, float* value)
{
    if (value == NULL) {
...
// BSP接口结构体
static const bsp_interface_t s_esp32_s3_devkit_lite_interface = {
    .init = esp32_s3_devkit_lite_init,
    .deinit = esp32_s3_devkit_lite_deinit,
    .get_board_info = esp32_s3_devkit_lite_get_board_info,
    .get_hw_config = esp32_s3_devkit_lite_get_hardware_config,
    
    // 传感器功能
    .sensor_init = esp32_s3_devkit_lite_sensor_init,
    .sensor_deinit = esp32_s3_devkit_lite_sensor_deinit,
    .sensor_read = esp32_s3_devkit_lite_sensor_read,
    
    // 蓝牙配网功能 - 临时禁用
    /*
    .bt_provision_init = esp32_s3_devkit_lite_bt_provision_init,
    .bt_provision_deinit = esp32_s3_devkit_lite_bt_provision_deinit,
    .bt_provision_start = esp32_s3_devkit_lite_bt_provision_start,
    .bt_provision_stop = esp32_s3_devkit_lite_bt_provision_stop
    */
};
...
hal_err_t bsp_esp32_s3_devkit_lite_register(void)
{
    ...
    return bsp_register_interface(&s_esp32_s3_devkit_lite_interface);
}
...
static hal_err_t esp32_s3_devkit_lite_led_set_brightness(uint8_t led_index, uint8_t brightness)
{
    ...
    if (led_index == 0 && LED1_PWM_ENABLED) {
        ...
        // 非PWM模式，只能开关控制
        return esp32_s3_devkit_lite_led_control(led_index, brightness > 127);
    }
#else
...
{
    return esp32_s3_devkit_lite_led_control(0, state);
}
...
{
    return esp32_s3_devkit_lite_led_set_brightness(0, brightness);
}
...
{
    return esp32_s3_devkit_lite_led_control(1, state);
}
...
{
    return esp32_s3_devkit_lite_led_set_brightness(1, brightness);
}
...
{
    return esp32_s3_devkit_lite_led_control(2, state);
}
...
{
    return esp32_s3_devkit_lite_led_set_brightness(2, brightness);
}
...
{
    return esp32_s3_devkit_lite_led_control(3, state);
}
...
{
    return esp32_s3_devkit_lite_led_set_brightness(3, brightness);
}
...
{
    return esp32_s3_devkit_lite_relay_control(0, state);
}
...
{
    return esp32_s3_devkit_lite_relay_control(1, state);
}
...
{
    return esp32_s3_devkit_lite_relay_control(relay_index, state);
}
...
{
    // Lite板子没有舵机，直接返回
    printf("BSP: Lite board has no servos, skipping servo initialization\n");
    return HAL_OK;
 ...
 * @brief 设置舵机1角度
 */
hal_err_t bsp_esp32_s3_devkit_lite_servo1_set_angle(uint16_t angle)
{
    return esp32_s3_devkit_lite_servo_set_angle(0, angle);
}

/**
 * @brief 设置舵机2角度
 */
hal_err_t bsp_esp32_s3_devkit_lite_servo2_set_angle(uint16_t angle)
{
    return esp32_s3_devkit_lite_servo_set_angle(1, angle);
}

/**
 * @brief 通用舵机控制函数
 */
hal_err_t bsp_esp32_s3_devkit_lite_servo_set_angle(uint8_t servo_index, uint16_t angle)
{
    return esp32_s3_devkit_lite_servo_set_angle(servo_index, angle);
}
