/**
 * @file bsp_esp32_s3_devkit.c
 * @brief ESP32-S3 DevKit 板级支持包实现
 * 
 * @author AIOT Team
 * @date 2024
 */

#include "bsp_esp32_s3_devkit.h"
#include "board_config.h"
#include "../../main/bsp/bsp_interface.h"
// #include "../../main/bluetooth/bt_provision.h"  // 临时禁用
#include "../../drivers/sensors/dht11.h"      // DHT11传感器驱动
#include "../../drivers/sensors/ds18b20.h"    // DS18B20传感器驱动
#include <stdio.h>
#include <string.h>
...
// 静态函数声明
static hal_err_t esp32_s3_devkit_init_leds(void);
static hal_err_t esp32_s3_devkit_init_buttons(void);
static hal_err_t esp32_s3_devkit_init_relays(void);
static hal_err_t esp32_s3_devkit_init_servos(void);
static hal_err_t esp32_s3_devkit_led_control(uint8_t led_index, bool state);
static hal_err_t esp32_s3_devkit_led_set_brightness(uint8_t led_index, uint8_t brightness);
static hal_err_t esp32_s3_devkit_relay_control(uint8_t relay_index, bool state);
static hal_err_t esp32_s3_devkit_servo_set_angle(uint8_t servo_index, uint16_t angle);

// 蓝牙配网功能函数声明 - 临时禁用
/*
static hal_err_t esp32_s3_devkit_bt_provision_init(const bt_provision_config_t* config);
static hal_err_t esp32_s3_devkit_bt_provision_deinit(void);
static hal_err_t esp32_s3_devkit_bt_provision_start(void);
static hal_err_t esp32_s3_devkit_bt_provision_stop(void);
*/
...
static hal_err_t esp32_s3_devkit_init(void)
{
    printf("BSP: Initializing ESP32-S3 DevKit...\n");
    
    // 1. 初始化LED GPIO
    hal_err_t ret = esp32_s3_devkit_init_leds();
    if (ret != HAL_OK) {
    ...
    // 2. 初始化按键GPIO
    ret = esp32_s3_devkit_init_buttons();
    if (ret != HAL_OK) {
    ...
    // 3. 初始化继电器GPIO
    ret = esp32_s3_devkit_init_relays();
    if (ret != HAL_OK) {
    ...
    // 4. 初始化舵机PWM
    ret = esp32_s3_devkit_init_servos();
    if (ret != HAL_OK) {
...
static hal_err_t esp32_s3_devkit_deinit(void)
{
...
static const bsp_board_info_t* esp32_s3_devkit_get_board_info(void)
{
    return &s_board_info;
}

static const bsp_hardware_config_t* esp32_s3_devkit_get_hardware_config(void)
{
    // 动态初始化硬件配置
...
// 传感器控制函数
static hal_err_t esp32_s3_devkit_sensor_init(void)
{
    printf("BSP: Initializing sensors (DHT11 and DS18B20)...\n");
    ...
    if (ds18b20_ret == ESP_OK) {
        printf("BSP: ✅ DS18B20 initialized successfully on GPIO%d\n", DS18B20_GPIO_PIN);
        ds18b20_ok = true;
    } else {
        printf("BSP: ⚠️ DS18B20 initialization failed (error: %d) - 将继续运行，DS18B20数据不可用\n", ds18b20_ret);
        // 不设置result = HAL_ERROR，允许其他传感器继续工作
    }
    
    // 只要至少有一个传感器成功，就返回HAL_OK
    if (dht11_ok || ds18b20_ok) {
        printf("BSP: ✅ Sensor initialization completed - DHT11: %s, DS18B20: %s\n", 
               dht11_ok ? "OK" : "Failed", ds18b20_ok ? "OK" : "Failed");
        result = HAL_OK;
    } else {
        printf("BSP: ⚠️ All sensors failed to initialize - 系统将继续运行\n");
        // 即使所有传感器都失败，也返回HAL_OK，不阻塞系统启动
        result = HAL_OK;
    }
...
static hal_err_t esp32_s3_devkit_sensor_deinit(void)
{
...
static hal_err_t esp32_s3_devkit_sensor_read(uint8_t sensor_id, float* value)
{
    if (value == NULL) {
...
// BSP接口结构体
static const bsp_interface_t s_esp32_s3_devkit_interface = {
    .init = esp32_s3_devkit_init,
    .deinit = esp32_s3_devkit_deinit,
    .get_board_info = esp32_s3_devkit_get_board_info,
    .get_hw_config = esp32_s3_devkit_get_hardware_config,
    
    // 传感器功能
    .sensor_init = esp32_s3_devkit_sensor_init,
    .sensor_deinit = esp32_s3_devkit_sensor_deinit,
    .sensor_read = esp32_s3_devkit_sensor_read,
    
    // 蓝牙配网功能 - 临时禁用
    /*
    .bt_provision_init = esp32_s3_devkit_bt_provision_init,
    .bt_provision_deinit = esp32_s3_devkit_bt_provision_deinit,
    .bt_provision_start = esp32_s3_devkit_bt_provision_start,
    .bt_provision_stop = esp32_s3_devkit_bt_provision_stop
    */
};
...
hal_err_t bsp_esp32_s3_devkit_register(void)
{
    ...
    return bsp_register_interface(&s_esp32_s3_devkit_interface);
}
...
static hal_err_t esp32_s3_devkit_led_set_brightness(uint8_t led_index, uint8_t brightness)
{
    ...
    if (led_index == 0 && LED1_PWM_ENABLED) {
        ...
        // 非PWM模式，只能开关控制
        return esp32_s3_devkit_led_control(led_index, brightness > 127);
    }
#else
...
{
    return esp32_s3_devkit_led_control(0, state);
}
...
{
    return esp32_s3_devkit_led_set_brightness(0, brightness);
}
...
{
    return esp32_s3_devkit_led_control(1, state);
}
...
{
    return esp32_s3_devkit_led_set_brightness(1, brightness);
}
...
{
    return esp32_s3_devkit_led_control(2, state);
}
...
{
    return esp32_s3_devkit_led_set_brightness(2, brightness);
}
...
{
    return esp32_s3_devkit_led_control(3, state);
}
...
{
    return esp32_s3_devkit_led_set_brightness(3, brightness);
}
...
{
    return esp32_s3_devkit_relay_control(0, state);
}
...
{
    return esp32_s3_devkit_relay_control(1, state);
}
...
{
    return esp32_s3_devkit_relay_control(relay_index, state);
}
...
{
    return esp32_s3_devkit_servo_set_angle(0, angle);
}
...
{
    return esp32_s3_devkit_servo_set_angle(1, angle);
}
...
{
    return esp32_s3_devkit_servo_set_angle(servo_index, angle);
}
